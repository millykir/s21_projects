...
void print_string(va_list *ap, params *variable, char *s, int *top);
void print_float(va_list *ap, params *variable, char *array, int *top);
void print_char(va_list *ap, params *variable, char *array, int *top);
void print_int(va_list *ap, params *variable, char *array, int *top,
               const char *gen_pointer);
void print_oct(va_list *ap, params *variable, char *array, int *top);
void print_p(va_list *ap, params *variable, char *array, int *top);
void print_x(va_list *ap, params *variable, char *array, int *top,
             const char *format);
void print_g(va_list *ap, params *variable, char *array, int *top,
             const char *format_pointer);
void print_exp(va_list *ap, params *variable, char *array, int *top,
               const char *format_pointer);
...
void add_space_precision_zero(char *temp, char *main_buf, int *top,
                              params *variable);
void long_char(wchar_t *long_str, params *variable, char *s, int *top);
void print_null(params *variable, char *s, int *top);
void normal_string(char *simple_str, params *variable, char *s, int *top);
void delete_zero(char *g_string, int *top);
int check_precision(params *variable, int expo);
int find_exp(long double num);
void get_star(va_list *ap, params *variable);
  ...
  for (gen_pointer = format; *gen_pointer; gen_pointer++) {
    ...
    if (!stop_flag) {
      ...
      if (s21_strchr("diu", (int)(*gen_pointer))) {
        print_int(&ap, &variable, s, &top_buffer, gen_pointer);
      } else if (*gen_pointer == 'c') {
        print_char(&ap, &variable, s, &top_buffer);
      } else if (*gen_pointer == 'E' || *gen_pointer == 'e') {
        print_exp(&ap, &variable, s, &top_buffer, gen_pointer);
      } else if (*gen_pointer == 'f') {
        print_float(&ap, &variable, s, &top_buffer);
      } else if (*gen_pointer == 'G' || *gen_pointer == 'g') {
        print_g(&ap, &variable, s, &top_buffer, gen_pointer);
      } else if (*gen_pointer == 'o') {
        print_oct(&ap, &variable, s, &top_buffer);
      } else if (*gen_pointer == 's') {
        print_string(&ap, &variable, s, &top_buffer);
      } else if (*gen_pointer == 'x' || *gen_pointer == 'X') {
        print_x(&ap, &variable, s, &top_buffer, gen_pointer);
      } else if (*gen_pointer == 'p') {
        variable.hash_zero = 1;
        print_p(&ap, &variable, s, &top_buffer);
...
// Флаги:
//[-, +, (пробел)][Ширина: (число)][Точность: .(число)][Длина: h, l, L]
...
void print_string(va_list *ap, params *variable, char *s, int *top) {
  ...
  if (variable->len_l == 1) {
    long_char(long_str, variable, s, top);
  } else {
    if (!simple_str) {
      print_null(variable, s, top);
    } else {
      normal_string(simple_str, variable, s, top);
    }
  }
}
...
void normal_string(char *simple_str, params *variable, char *s, int *top) {
  ...
  if ((variable->precision_num == -1 &&
       variable->dot_check == 1)) {  // точность 0 значит строка пустая
    len = 0;
    reserve[0] = '\0';
    // printf("+++++++++++++++++++++++++++++++++\n%s %d %d\n", reserve,
    // variable->precision_num, variable->dot_check);
  }
...
void print_null(params *variable, char *s, int *top) {
  ...
  if ((variable->width > 0) && (variable->align_minus == 0)) {
    ...
    for (int k = 0; k < variable->width; k++) {  // если
      s[*top] = ins;
      *top = (*top) + 1;
    }
  }
...
void print_int(va_list *ap, params *variable, char *array, int *top,
  ...
  int fill = 0;
  if (variable->precision_num >= 5) fill = 1;
  // int fill = variable->width-(int)s21_strlen(temp);
  // align_left(array, top, (int)s21_strlen(temp), variable->width-fill,
  // variable, // +1 no
  //              temp);
  if (variable->align_minus) {
    align_left(array, top,
               (int)s21_strlen(temp) + variable->precision_zero + fill,
...
// добавление пробелов справа - выравниыание слева
void align_left(char *main_buf, int *top, int len, int fill, params *variable,
                const char *temp) {
...
// float в строку
// char *float_to_string(long double num, params *variable) {
char *float_to_string(long double num, int g_flag, params *variable) {
  ...
  if (num != 0 && log10(num) >= 0) {  // выводим целую часть
  ...
  res[top++] = conflicting_char;  // точка
  long double int_part;
  long double frac_part = modfl(num, &int_part);
  if (check_fraction(&top, variable, num)) {  // если есть дробная часть
    int indicator = 0;
    long double next;
    for (int k = 0; k < (variable->precision_num + 1); k++) {
      frac_part *= 10;
...
int check_fraction(int *top, params *variable,
                   long double num) {  // есть ли дробная часть
  int total;
...
void print_p(va_list *ap, params *variable, char *array, int *top) {
  ...
  // надо добавить ведущие нули
  if (variable->precision_num > 0) {
    char *temp_ar = hex_to_string(convert, 0, variable->hash_zero, 0);
  ...
  char *temp = s21_NULL;
  if (variable->precision_zero) {
    temp = hex_to_string(convert, 0, 0, nil_flag);
  } else {
    temp = hex_to_string(convert, 0, variable->hash_zero, nil_flag);
  }
  ...
  if (variable->precision_zero) {
    ...
    add_precision_zero(&temp, array, top, variable, 1, &temp_shift);

  } else if (variable->direct_zero) {
    add_zero(&temp, array, top, (int)s21_strlen(temp), variable, &temp_shift);
  } /*else if (variable->width > 0 && variable->align_minus == 0) {
    add_space(temp, array, top, (int)s21_strlen(temp), variable);
  }*/

  write_into_main_buf(temp, array, top, (int)s21_strlen(temp), 0, 0);
  if (variable->align_minus) {
    align_left(array, top, (int)s21_strlen(temp), variable->width, variable,
               temp);
  }
