...
// конвертирует строковое представление числа, которое хранится в строке start,
// в длинное целое и возвращает результат
long s21_strtol(char *str, char **strend, int sys);
// преобразует начальную часть строки nptr в число с типом unsigned long int
// согласно системе счисления base, значение которой может быть от 2 до 36
// включительно или равно специальному значению 0
unsigned long s21_strtoul(char *str, char **strend, int sys);
// преобразование строки в число типа long double.
long double s21_strtold(char *str, char **strend);
// возвращает значение числа с плавающей запятой, за исключением случаев, когда
// представление вызовет переполнение.
  ...
  for (; *format; format++) {
    ...
    if (fl.form) {
      if ((int)*format >= '0' && (int)*format <= '9') {
        fl.width = fl.width * 10 + (int)*format - '0';
        continue;
      }
  ...
  if (s21_strtoul(*strc, s21_NULL, 16) ||
    ...
    if (!fl->supr) {
      ...
      *(void **)va_arg(*args, void **) =
          (void *)s21_strtoul(*strc, &strend, 16);
    } else {
