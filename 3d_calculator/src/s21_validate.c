  ...
  while (runner != NULL) {
    if (first) {
      if (runner->type == enum_number || runner->type == enum_minus ||
          runner->type == enum_x || runner->type == enum_division)
        error |= EXIT_SUCCESS;
      else
        error |= EXIT_FAILURE;
      first = 0;
    ...
    if (runner->type == enum_number) {
      ...
      if (runner->NEXT == NULL)
        ...
        error |= EXIT_FAILURE;
      else if ((runner->NEXT)->type == enum_division)
        error |= EXIT_FAILURE;
    }
    ...
    else if (runner->type == enum_division) {
      if (runner->NEXT == NULL)
        error |= EXIT_FAILURE;
      else if ((runner->NEXT)->type == enum_number)
        error |= EXIT_SUCCESS;
